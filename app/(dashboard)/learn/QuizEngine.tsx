...
export default function QuizEngine({
  ...
  if (showResult) {
    return (
      <div className="bg-white p-8 rounded-3xl shadow-lg border border-slate-100 text-center space-y-6">
        ...
        <div>
          ...
          <p className="text-slate-500 mt-2">{t.totalScore}</p>
          <p className="text-5xl font-extrabold text-slate-900 mt-2">{score}</p>
        </div>
        <button
          onClick={restartQuiz}
          className="w-full py-4 bg-slate-900 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-black transition"
        >
  ...
  // SORU EKRANI
  return (
    <div className="bg-white p-6 rounded-3xl shadow-lg border-t-4 border-purple-500 min-h-[400px] flex flex-col justify-between">
      <div>
        ...
        <h3 className="font-bold text-xl text-slate-900 mb-6 leading-snug">
      ...
      {isAnswered && (
        <button
          ...
          className="mt-6 w-full py-4 bg-purple-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-purple-700 transition animate-in fade-in slide-in-from-bottom-2"
        >
